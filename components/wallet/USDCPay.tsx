...
export default function USDCPay({ defaultAmount = 1000 }: { defaultAmount?: number }) {
  ...
  return (
    <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/10 p-6 glass glow-emerald">
      <h3 className="mb-4 text-xl font-semibold text-white">Pay with USDC (Base)</h3>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3">
        ...
        <input
          ...
          className="w-32 rounded-lg bg-black/40 px-3 py-2 text-white outline-none border border-white/20 focus:border-su-emerald transition"
        />
        <span className="text-white/70 font-medium">USDC</span>
      </div>
      
      <div className="flex flex-wrap gap-3">
        <a
          href={link}
          className="rounded-lg bg-su-emerald px-6 py-3 font-semibold text-black hover:opacity-90 transition inline-flex items-center gap-2"
        >
        ...
        <button
          ...
          className="rounded-lg border border-white/20 px-6 py-3 text-white hover:bg-white/10 transition inline-flex items-center gap-2"
        >
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
